...
interface ToolkitProps {
    ...
    onLassoPicksShapesChange?: (picksShapes: boolean) => void
}
...
export default function Toolkit({
    ...
    onLassoPicksShapesChange
}: ToolkitProps) {
    const toolkitRef = useRef<HTMLDivElement>(null)
    const colorInputRef = useRef<HTMLInputElement>(null)
    const [openPopup, setOpenPopup] = useState<'pen' | 'eraser' | 'figures' | 'select' | 'lasso' | null>(null)
    ...
    const handleToolClick = (tool: ToolType) => {
        if (tool === 'pen') {
            ...
            if (activeTool === 'lasso') {
                setOpenPopup(openPopup === 'lasso' ? null : 'lasso')
            } else {
                onToolChange('lasso')
                setOpenPopup(null)
            }
            ...
            {openPopup === 'lasso' && (
                ...
                >
                    <div style={{ padding: '4px 10px' }}>
                        <button
                            onClick={() => onLassoPicksShapesChange?.(!lassoPicksShapes)}
                            style={{
                                width: '100%',
                                padding: '10px',
                                display: 'flex',
                                ...
                                justifyContent: 'center',
                                gap: 10,
                                background: lassoPicksShapes ? (activeColor || '#007AFF') : '#f5f5f7',
                                color: lassoPicksShapes ? '#fff' : '#555',
                                border: 'none',
                                borderRadius: 12,
                                cursor: 'pointer',
                                fontSize: 13,
