...
export default function NotebookView() {
  ...
  const contentBounds = useMemo(() => getContentBounds(pages), [pages]);
  ...
  const [isShapeFilled, setIsShapeFilled] = useState(() => {
    const saved = localStorage.getItem(TOOLKIT_STORAGE_KEY);
    return saved && JSON.parse(saved).isShapeFilled !== undefined
      ? JSON.parse(saved).isShapeFilled
      : true;
  });
  const [lassoPicksShapes, setLassoPicksShapes] = useState(() => {
    const saved = localStorage.getItem(TOOLKIT_STORAGE_KEY);
    return saved && JSON.parse(saved).lassoPicksShapes !== undefined
      ? JSON.parse(saved).lassoPicksShapes
      : false;
  });
  ...
  const [pdfActions, setPdfActions] = useState<{
    undo: () => void;
    redo: () => void;
    canUndo: boolean;
    canRedo: boolean;
  ...
  const isContentOffscreen = useIsContentOffscreen(
    pages,
    viewport,
    canvasSize.width,
    canvasSize.height,
  ...
  useEffect(() => {
    ...
    Promise.all([getNotebook(notebookId), getPages(notebookId)]).then(
      async ([nb, p]) => {
        ...
        if (notebook && notebook.pdfIds) {
          ...
          const pdfFiles = await Promise.all(
            notebook.pdfIds.map((id) => getPDFFile(id)),
          );
          const pdfTabs: Tab[] = [];
  ...
  useEffect(() => {
    ...
    if (
      ...
      pages.length === 0 ||
      !notebook ||
      hasAttemptedInitialScroll.current
    )
      return;
    ...
    const scrollTimeout = setTimeout(() => {
      ...
      if (!targetEl) {
        targetEl = document.getElementById(
          `page-wrapper-${pages[pages.length - 1].id}`,
        );
      }
      ...
      if (targetEl) {
        ...
        // Use our Hook's internal viewport state instead of native browser scroll.
        // This is only the initial notebook positioning, so reset zoom too.
        setViewport(1, 0, -topOffset);
        hasAttemptedInitialScroll.current = true;
      }
    }, 300);

    return () => clearTimeout(scrollTimeout);
  }, [loading, notebook, pages, activeTabId, setViewport]);
  ...
  async function handleAddPage(template: PageTemplate) {
    ...
    setNotebook(updated);
    setPages((prev) =>
      [...prev, page].sort((a, b) => a.createdAt - b.createdAt),
    );
  ...
  async function handleExport(specificPages?: Page[]) {
    if (
      !notebook ||
      (specificPages ? specificPages.length === 0 : pages.length === 0)
    )
      return;
    setIsExporting(true);
    try {
  ...
  return (
    ...
    >
            ...
            <button
              ...
              onClick={async () => {
                if (notebook && notebook.folderId) {
                  ...
                  if (folder) {
                    navigate(
                      `/workspace/${folder.workspaceId}/folder/${folder.id}`,
                    );
                    return;
                  }
                }
                ...
                {pages.map((p, i) => (
                  <div
                    ...
                    style={{
                      ...
                      padding: "8px",
                      borderRadius: "16px",
                      background: selectedPageIds.includes(p.id)
                        ? "#f8fafc"
                        : "transparent",
      ...
      <Toolkit
        ...
        onUndo={
          activeTabId !== "notes" && pdfActions ? pdfActions.undo : handleUndo
        }
        onRedo={
          activeTabId !== "notes" && pdfActions ? pdfActions.redo : handleRedo
        }
        canUndo={
          activeTabId !== "notes" && pdfActions
            ? pdfActions.canUndo
            : undoStack.length > 0
        }
        canRedo={
                      ...
                      >
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            setActiveMenuPageId(
                              activeMenuPageId === p.id ? null : p.id,
                            );
                          }}
                          style={{
                            background: "none",
              ...
              <div style={{ position: "relative" }}>
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    setActiveTabMenuId(
                      activeTabMenuId === tab.id ? null : tab.id,
                    );
                  }}
                  style={{
                    width: "18px",
